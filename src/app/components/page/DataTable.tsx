        ...
        {data.length > 0 ? (
          data.map((row) => (
            <tr key={row.id} className={`bg-gray-100`}>
              ...
              <td className={`py-2 px-4 border-b ${row.status === 'empty' ? 'bg-yellow-500' : ''}`}>{row.status}</td>
              <td className={`py-2 px-4 border-b ${row.status === 'empty' ? 'bg-yellow-500' : ''}`}>{moment(row.fill_date).format('DD/MM/YYYY')}</td>
