...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Base query
    let query = `SELECT cm.*, l.name as name_location, s.name as name_shelve, b.name as name_bin, bsm.tat_sku AS tat_sku, bsm.container_rfid AS bin_stock_container_id FROM container_management cm JOIN location l ON cm.location_id = l.id JOIN shelve s ON cm.shelve_id = s.id JOIN bin b ON cm.bin_id = b.id LEFT JOIN bin_stock_management bsm ON cm.container_id = bsm.container_rfid`;
